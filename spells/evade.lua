...
local menu_elements = {
    ...
    debug_enabled = checkbox:new(true, get_hash(my_utility.plugin_label .. "debug_enabled_evade")) -- Enable debug by default for troubleshooting
}
...
local function update_charges()
    ...
    if current_time >= charge_refresh_time then
        ...
        if menu_elements.debug_enabled:get() then
            console.print(string.format("Evade: Charge refreshed. Current charges: %d/%d", current_charges, max_charges))
        end
    end
end
...
local function cast(target_selector_data, best_ranged_target, best_melee_target, best_cursor_target, closest_cursor_target, closest_cursor_target_angle)
    ...
    if current_charges <= min_charges then
        if menu_elements.debug_enabled:get() then
            console.print(string.format("Evade: Not enough charges (%d/%d, minimum %d)", 
                current_charges, max_charges, min_charges))
        end
        return false
    end
    
    if not utility.is_spell_ready(evade_spell_id) then return false end
    ...
    if not target then
        if debug_enabled then
            console.print("Evade: No valid target found")
        end
        return false
    end
    ...
    if menu_elements.mobility_only:get() then
        ...
        if distance > max_spell_range or distance < min_range then
            if debug_enabled then
                console.print(string.format("Evade: Target distance not suitable for mobility (%.2f)", distance))
            end
            return false
        end
    end
    ...
    -- Cast the spell
    if cast_spell.position(evade_spell_id, target_pos, 0.0) then
        ...
        if debug_enabled then
            console.print(string.format("Evade: Cast to target at distance %.2f (Mobility Only: %s, Charges: %d/%d)", 
                distance, tostring(menu_elements.mobility_only:get()), current_charges, max_charges))
        end
...
local function out_of_combat()
    ...
    if current_charges <= min_charges then
        if menu_elements.debug_enabled:get() then
            console.print(string.format("Evade: Not enough charges for OOC (%d/%d, minimum %d)", 
                current_charges, max_charges, min_charges))
        end
        return false
    end
    ...
    if not in_combat_area then
        if debug_enabled then
            console.print("Evade: Not in combat area")
        end
        return false
    end
    ...
    if not is_moving then
        if debug_enabled then
            console.print("Evade: Not moving")
        end
        return false
    end
    ...
    if targeting_mode == 2 or targeting_mode == 3 then -- Cursor modes
        ...
        if cast_spell.position(evade_spell_id, destination, 0.0) then
            next_time_allowed_cast = current_time + menu_elements.min_cooldown:get()
            last_cast_time = current_time
            ...
            if debug_enabled then
                console.print(string.format("Evade: Cast to cursor position (Out of Combat, Charges: %d/%d)", 
    ...
    if is_dashing then
        if debug_enabled then
            console.print("Evade: Already dashing")
        end
        return false
    end
    ...
    if distance >= min_evade_distance then
        if cast_spell.position(evade_spell_id, destination, 0.0) then
            ...
            if debug_enabled then
                console.print(string.format("Evade: Cast to move destination at distance %.2f (Out of Combat, Charges: %d/%d)", 
                    distance, current_charges, max_charges))
            end
        ...
        if debug_enabled then
            console.print(string.format("Evade: Move destination too close (%.2f < %.2f)", distance, min_evade_distance))
        end
    end
...
local function logics(target)
    ...
    if not menu_elements.main_boolean:get() then
        if debug_enabled then console.print("Evade: Disabled in menu") end
        return false
    end
    ...
    if current_time < next_time_allowed_cast then
        if debug_enabled then console.print("Evade: On cooldown") end
        return false
    end
    ...
    if current_charges <= min_charges then
        if debug_enabled then
            console.print(string.format("Evade: Not enough charges for logics (%d/%d, minimum %d)", 
                current_charges, max_charges, min_charges))
        end
        return false
    end
    
    if not utility.is_spell_ready(evade_spell_id) then
        if debug_enabled then console.print("Evade: Spell not ready") end
        return false
    end
    ...
    if not target or not target.get_position then
        if debug_enabled then console.print("Evade: Invalid target, using cursor position") end
    ...
    if cast_spell.position(evade_spell_id, target_pos, 0.0) then
        ...
        -- Print momentum stacking message if not fully stacked
        local local_player = get_local_player()
        if local_player and not _G.initial_momentum_stacked then
            console.print("Evade: Used for momentum stacking")
        end
        
        return true
    end
    
    if debug_enabled then console.print("Evade: Failed to cast") end
    return false
end
